...
@interface TUISearchViewController () <UITableViewDelegate, UITableViewDataSource, TUISearchBarDelegate, TUISearchResultDelegate, UIGestureRecognizerDelegate>

@property (nonatomic, strong) UIView *cusTitleView;
...
- (void)setupViews
{
    ...
    _searchBar = [[TUISearchBar alloc] initWithFrame:CGRectMake(0, StatusBar_Height, self.view.mm_w, 44)];
    ...
    [_searchBar.searchTF becomeFirstResponder];
}
    ...
    if (module == TUISearchResultModuleChatHistory) {
        ...
        if (msgs.count == 1) {
            NSDictionary *param = @{TUICore_TUIChatService_GetChatViewControllerMethod_TitleKey : cellModel.title ?: @"",
                                    ...
                                    TUICore_TUIChatService_GetChatViewControllerMethod_HighlightKeywordKey : self.searchBar.searchTF.text ?: @"",
        ...
        for (V2TIMMessage *message in msgs) {
            ...
            NSString *desc = [TUISearchDataProvider matchedTextForMessage:message withKey:self.searchBar.searchTF.text];
            model.detailsAttributeString = [TUISearchDataProvider attributeStringWithText:desc key:self.searchBar.searchTF.text];
        ...
        TUISearchResultListController *vc = [[TUISearchResultListController alloc] initWithResults:results
                                                                                           keyword:self.searchBar.searchTF.text
                                                                                            module:module
